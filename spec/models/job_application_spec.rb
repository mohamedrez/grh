...
RSpec.describe JobApplication, type: :model do
  ...
  describe '#applicant_state' do
    it 'returns the correct hash for different states' do
      ...
      expect(applied_state).to eq({ second_state: "advanced_to_phone", title: I18n.t("attributes.job_application.aasm_states.advance_to_phone") })

      advanced_to_phone_state = job_application.applicant_state("advanced_to_phone")
      expect(advanced_to_phone_state).to eq({ second_state: "completed_phone", title: I18n.t("attributes.job_application.aasm_states.complete_phone") })

      completed_phone_state = job_application.applicant_state("completed_phone")
      expect(completed_phone_state).to eq({ second_state: "advanced_interview", title: I18n.t("attributes.job_application.aasm_states.advance_to_interview") })

      advanced_interview_state = job_application.applicant_state("advanced_interview")
      expect(advanced_interview_state).to eq({ second_state: "completed_interview", title: I18n.t("attributes.job_application.aasm_states.complete_interview") })
