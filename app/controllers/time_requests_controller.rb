class TimeRequestsController < ApplicationController
  ...
  def create
    ...
    if @time_request.save
      redirect_to user_time_request_url(@user, @time_request), notice: t("time_requests.time_requests_created")
    else
  ...
  def update
    ...
    if @time_request.update(time_request_params)
      redirect_to user_time_request_url(@user, @time_request), notice: t("time_requests.time_requests_updated")
    else
  ...
  def destroy
    ...
    flash.now[:notice] = t("time_requests.time_requests_destroyed")
    render turbo_stream: [
